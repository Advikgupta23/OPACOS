...
class Kroupa(MassFunction):
    ...
    def __init__(self,
        ...
        #self.distr = distributions.BrokenPowerLaw([-p1,-p2,-p3,-p4],
        #                                       [0.03, break1, break2, break3, 120])
        
        if self.mmin < break1 and self.mmax > break3:
           self.distr = distributions.BrokenPowerLaw([-p1, -p2, -p3, -p4],
                                                  [self.mmin, break1, break2, break3, self.mmax])
        elif break1 <= self.mmin < break2 and self.mmax >= break3:
             self.distr = distributions.BrokenPowerLaw([-p2, -p3, -p4],
                                                  [self.mmin, break2, break3, self.mmax])
        elif self.mmin < break1 and break2 < self.mmax <= break3:
             self.distr = distributions.BrokenPowerLaw([-p1, -p2, -p3],
                                                  [self.mmin, break1, break2, self.mmax])                        
        elif self.mmin < break1 and break1 < self.mmax <= break2:
             self.distr = distributions.BrokenPowerLaw([-p1, -p2],
                                                  [self.mmin, break1, self.mmax])
        elif break1 <= self.mmin < break2 and break2 < self.mmax <= break3:
             self.distr = distributions.BrokenPowerLaw([-p2, -p3],
                                                  [self.mmin, break2, self.mmax])
        elif break2 <= self.mmin < break3 and self.mmax > break3:
             self.distr = distributions.BrokenPowerLaw([-p3, -p4],
                                                  [self.mmin, break3, self.mmax])
        elif self.mmin <= break1 and self.mmax <= break1:
             self.distr = distributions.BrokenPowerLaw([-p1],
                                                  [self.mmin, self.mmax])
        elif break1 <= self.mmin <= break2 and break1 <= self.mmax <= break2:
             self.distr = distributions.BrokenPowerLaw([-p2],
                                                  [self.mmin, self.mmax])
        elif break2 <= self.mmin <= break3 and break2 <= self.mmax <= break3:
             self.distr = distributions.BrokenPowerLaw([-p3],
                                                  [self.mmin, self.mmax])                                    
        elif break3 <= self.mmin and break3 <= self.mmax:
             self.distr = distributions.BrokenPowerLaw([-p4],
                                                  [self.mmin, self.mmax])
...
class ChabrierPowerLaw(MassFunction):
    ...
    def __init__(self,
        ...
        if self.mmax <= self._mmid:
           "The Chabrier Mass Function does not support mmax <= mmid "
...
under the hood.
    ...
    if get_massfunc_name(massfunc) == 'chabrierlognormal':
       if Mmax <= 1.0: 
          return mfc.distr.ppf(p)
       if Mmin >=1.0:
          return salpeter.distr.ppf(p)
       else:
          return mfc.distr.ppf(p)
             
    if get_massfunc_name(massfunc) == 'kroupa':
